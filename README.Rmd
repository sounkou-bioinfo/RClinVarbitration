...
Native builds currently support Linux and macOS and require libxml2, zlib,
`pkg-config`, and a C compiler. `OS_type: unix` prevents an unsupported Windows
installation; a Windows DuckDB-extension build has not yet been implemented.
webR is supported separately through the tested Emscripten build and is
Unix-like for R package metadata purposes.
...
See the [function reference](https://sounkou-bioinfo.github.io/RClinVarbitration/reference/index.html)
...
For the closest algorithm-level comparison, use the archived NCBI flat files:
