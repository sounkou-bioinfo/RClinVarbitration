...
otherwise the executable document uses a temporary database. On first use,
`rclinvarbitration_enable()` installs DuckDB's official, version-matched JSON
extension when necessary. The connection therefore needs permission to download
DuckDB extensions once; a restricted environment can run `INSTALL json` on the
